...
def login():
    ...
    if request.method == "POST":
        ...
        cur.execute("SELECT * FROM users WHERE username = %s AND passw = %s", (uname, passw))
        data = cur.fetchone()
        cur.close()
        
        # Check if no data is available in the database
        if data is None:
            error = "Data not available or invalid"
        else:
            # Check the user name index value from the database
            if uname == data[4] and passw == data[5]:
...
def register():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO users (name, phno, gender, username, passw) VALUES (%s, %s, %s, %s, %s)", 
                    (name, phno, gender, username, password))
